...
def validate(request, number):
    try:
        ...
        if account:
            ...
            reports = Report.objects.filter(account_number=account.number)
            # Get report count
            ...
            if(ordered_reports.count() >= 2):
                ...
                rate = abs(reported_count/((latest.datetime - second_latest.datetime).days))
            ...
            return Response(data={'account': serializer.data, 'score': score, 'total_reports': reported_count,},status=status.HTTP_200_OK)
        else:
    ...
    def post(self,request):
        ...
        try:
            ...
            if data.get("evidence") is not None:
                report_data['evidence'] = data['evidence']
            ...
            account = Account.objects.filter(number=account).first()
            ...
            return Response(data={'report':serializer.data, 'account': acc_serializer.data,  'score': score, 'total_reports': reported_count,},status=status.HTTP_201_CREATED)
        except Exception as e:
            print(e)
            return Response(status=status.HTTP_400_BAD_REQUEST)
